...
3. Supports automated figure summarization with OpenAI models
...
# OpenAI imports for figure summarization
try:
    import openai
...
class EnhancedMarkdownProcessor:
    ...
    def __init__(
        ...
        Args:
            ...
            openai_api_key: OpenAI API key for figure summarization
        ...
        if OPENAI_AVAILABLE and openai_api_key:
            ...
            self.logger.info("OpenAI client initialized for figure summarization")
        elif enable_ai_summarization:
            self.logger.warning("AI summarization requested but OpenAI not available")
...
"""
    ...
    def _summarize_figure_with_ai(self, figure_path: Path, figure_metadata: FigureMetadata) -> Optional[str]:
        """Generate AI summary for a figure using OpenAI Vision models."""
...
Provide a clear, factual summary in 2-3 sentences."""
            ...
            self.logger.debug(f"Making OpenAI API call for {figure_metadata.figure_id}")
            api_start = time.time()
    ...
    def _create_enhanced_markdown(
        ...
        if self.enable_ai_summarization and self.openai_client:
            ai_start = time.time()
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python enhanced_markdown_processor.py <pdf_path> [openai_api_key]")
        sys.exit(1)
    ...
    processor = create_enhanced_processor(
        ...
        enable_ai_summarization=bool(openai_key)
    )
